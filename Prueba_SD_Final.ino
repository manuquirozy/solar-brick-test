...
unsigned long tres  = 0;                                 //timing variable for one status of the FSM
...
void setup() { //begin of void set up
  ...
  dataFile = SD.open("datalog.txt", FILE_WRITE);                                                              //opens or intializies the file, dependent if that file already exits or not
  if (dataFile) {
...
void loop() { //begin of void loop
  ...
  delay(10);
  digitalWrite(output_control, LOW);
  for (int i = 0 ;  i  <  0.5 * 15 ; i++) // Ese 15: ...*60/4
  {
